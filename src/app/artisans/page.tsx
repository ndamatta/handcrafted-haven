...
export default async function ArtisansPage({
  ...
  return (
    <div className="font-sans min-h-screen flex flex-col bg-gradient-to-b from-white to-gray-50 dark:from-[#18181b] dark:to-[#23232a]">
      <Header isLoggedIn={!!session} />
      <Container>
        <section id="artisans" className="py-10">
          ...
          {totalArtisans === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-10">
              {artisans.map((artisan: SafeUser & { product_count: number; average_rating: number | null }) => (
                <Link key={artisan.id} href={`/artisans/${createArtisanSlug(artisan.name)}`} className="hover:scale-105 transition-transform">
          ...
          {totalArtisans > pageSize && (
            <div className="flex justify-center items-center gap-3 text-sm mb-8">
              {prevPage && (
                <Link
                  href={`/artisans${buildQueryString({ page: prevPage.toString() })}`}
                  className="px-3 py-1 bg-slate-200 dark:bg-zinc-700 rounded-md hover:bg-slate-300 dark:hover:bg-zinc-600 transition-colors"
                >
                  ← Previous
                </Link>
              )}
              ...
              {nextPage && (
                <Link
                  href={`/artisans${buildQueryString({ page: nextPage.toString() })}`}
                  className="px-3 py-1 bg-slate-200 dark:bg-zinc-700 rounded-md hover:bg-slate-300 dark:hover:bg-zinc-600 transition-colors"
                >
                  Next →
                </Link>
